...
public class PlayerCraftStation : CraftStationInventory
{
    ...
    #region Open/Close
    new public void CloseCraftStation()
    {
        craftStationOpened = false;
    }

    new public void OpenCraftStation(Slider _craftProgressBar)
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            if (items[i].GetItem() == itemInProgress)
            {
                ...
                if (quantityLeft != 0)
                {
                    ...
                    {
                        int add = quantityLeft - items[i].GetItem().maxStackSize;
                        amountPerCraft -= add;
                        items[i].AddQuantity(add);
                    }
                }
            }
        }

        if (_amountPerCraft != 0)
        {
            for (int i = 0; i < items.Length; i++)
            {
                if (items[i].GetItem() == null)
                {
                    items[i].AddItem(itemInProgress, _amountPerCraft);
                    hasSpace = true;
                    _amountPerCraft = 0;
                    break;
                }
    ...
    new public bool TryToPutItemInQueue(CraftableItemClass _item, int _craftAmount)
    {
    ...
    new public void OnCraft(CraftableItemClass _item, int _craftAmount)
    {
    ...
    new public void SummarizeNeededItemsForQueue()
    {
    ...
    new public void SummarizeNeededItems()
    {
        if (craftingQueue[0].GetItem() == null)
        {
            neededItems.Clear();
    ...
    new public void SummarizeItemsInInventory()
    {
    ...
    new public void GetListOfLeftItems()
    {
        SummarizeItemsInInventory();
        SummarizeNeededItemsForQueue();
    ...
    new public void CancelQueue(bool clearCurrentItem)
    {
    ...
    new public bool CheckIfQueueSlotIsCraftable(ItemClass _item, int queueSlotIndex, List<SlotClass> _neededItemsCopy, List<SlotClass> _sumInventoryItemsCopy)
    {
        //check through all needed items for crafting this item
        for (int x = 0; x < craftingQueue[queueSlotIndex].GetItem().GetNeededItems().Length; x++)
        {
    ...
    new public void GetMaxCraftAmountForRequestedItem(CraftableItemClass _craftableItem)
    {
        GetListOfLeftItems();
